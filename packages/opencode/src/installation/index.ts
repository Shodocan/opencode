...
//   { version: "1.18.28-RC1", url: "<tarball url>", sha256: "<hex>" }
// Version scheme: clean release = X.Y.Z; bug-fix republishes on the same
// release = X.Y.Z-RC1, -RC2, … The auto-update gate is string-equality +
// major/minor compare (see cli/upgrade.ts), so X.Y.Z → X.Y.Z-RCn
// correctly triggers a patch upgrade.
    ...
    const upgradeCustomMinio = Effect.fnUntraced(
      function* (_target: string) {
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get(CUSTOM_MANIFEST_URL).pipe(HttpClientRequest.acceptJson),
        )
        ...
        try {
          ...
          const srcBin =
            found ||
            (yield* text(["sh", "-c", `find "${extractDir}" -maxdepth 3 -type f -name opencode | head -n1`])).trim()
