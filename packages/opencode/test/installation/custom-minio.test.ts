...
describe("custom-minio install detection", () => {
  ...
  test("customLibDir derives LIB_DIR from execPath", () => {
    expect(Installation.customLibDir(CUSTOM_EXEC)).toBe("/home/user/.opencode-custom-hindsight/lib/opencode-custom")
  ...
  testEffect(
    testLayer((request) => {
      if (request.url.includes("/opencode-custom/manifest.json"))
        return jsonResponse({ version: "1.18.28-RC2", url: "https://x/tar.gz", sha256: "abc" })
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        expect(result).toBe("1.18.28-RC2")
      }),
    ),
  )
})
...
describe("custom-minio upgrade()", () => {
  ...
  testEffect(
    ...
    Effect.scoped(
      Effect.gen(function* () {
        yield* withExecPath(CUSTOM_EXEC)
        yield* Installation.use.upgrade("curl", "1.18.28-RC2")
...
describe("RC version scheme triggers patch upgrades", () => {
  ...
  // (major/minor compare). Verify X.Y.Z -> X.Y.Z-RCn is a patch upgrade.
  test("getReleaseType: clean release to RCn is a patch", () => {
    expect(Installation.getReleaseType("1.18.28", "1.18.28-RC1")).toBe("patch")
    expect(Installation.getReleaseType("1.18.28-RC1", "1.18.28-RC2")).toBe("patch")
    expect(Installation.getReleaseType("1.18.28-RC2", "1.18.29")).toBe("patch")
    expect(Installation.getReleaseType("1.18.28", "1.19.0")).toBe("minor")
    expect(Installation.getReleaseType("1.18.28", "2.0.0")).toBe("major")
  })
})
