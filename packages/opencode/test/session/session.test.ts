...
describe("session.created event", () => {
  ...
  it.instance("propagates parent origin metadata through durable create/projector/notification seam", () =>
    Effect.gen(function* () {
      ...
      // Create child session with parent origin metadata through the internal
      // host path; public create cannot mint host-owned opencode.* metadata.
      const created = yield* session.createTaskChild({
        parentID: parent.id,
        title: "task-child",
        agent: "general",
        permission: [],
