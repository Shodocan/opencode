...
const parent = SessionID.make("ses_before_chain_parent")
const call = "before-chain-call"
...
const it = testEffect(Layer.mergeAll(
  ...
  Layer.mock(ToolRegistry.Service, { tools: () => Effect.succeed([{
    id: "task", description: "Must never execute after admission rejection", parameters: Schema.Struct({}),
    jsonSchema: { type: "object", properties: {} }, execute: () => Effect.sync(() => {
      executions++
      return { title: "unexpected native launch", metadata: {}, output: "unexpected" }
    }),
  }]) }),
))

it.instance("a later plugin before failure releases the real workflow claim with native not-started proof", () => Effect.gen(function* () {
  ...
  const second = path.join(fixture.directory, "rejecting-plugin.ts")
  ...
  yield* Effect.promise(() => Bun.write(second,
    ...
    `'tool.execute.before': async input => {if(input.tool==='task')throw new Error('later plugin admission rejection');},\n` +
  ...
  yield* Effect.promise(() => Bun.write(path.join(fixture.directory, "opencode.json"), JSON.stringify({
    $schema: "https://opencode.ai/config.json", plugin: [pathToFileURL(first).href, pathToFileURL(second).href],
  })))
  const plugins = yield* Plugin.Service
  const hooks = yield* plugins.list()
  const workflowHooks = hooks.find(hook => hook.tool?.workflow)
  expect(workflowHooks, "real workflow plugin must load before the rejecting plugin").toBeDefined()
  ...
  const start: any = yield* Effect.promise(() => workflowHooks!.tool!.workflow.execute({ action: "start", workflow,
    args: { request: "Read-only regression for a later plugin denying an already claimed native Task." } }, context as never))
  ...
  expect(result).toContain("later plugin admission rejection")
  ...
  expect(attempt.nativeNotStarted, "native rejection before execution must release the already durable claim").toBe(true)
