...
const model = { providerID: ProviderV2.ID.make("opencode-retry"), modelID: ModelV2.ID.make("workflow-child") }
  ...
  for (const managed of [true, false]) {
    it.live(`${native ? "native" : "SDK"} HTTP retry budget: ${managed ? "managed Task sends once" : "ordinary Task retains retries"}`, () =>
      provideTmpdirServer(({ llm }) => Effect.gen(function* () {
        ...
        const selected = yield* provider.getModel(model.providerID, model.modelID)
        const catalog = yield* provider.getProvider(model.providerID)
        expect(LLMNativeRuntime.status({ model: selected, provider: catalog, auth: undefined }).type).toBe("supported")
        ...
        model: "opencode-retry/workflow-child", small_model: "opencode-retry/auxiliary",
        provider: { "opencode-retry": { npm: "@ai-sdk/openai-compatible", options: { baseURL: url, apiKey: "local-test" },
          models: {
