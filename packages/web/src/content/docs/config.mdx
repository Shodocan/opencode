...
- `fallback_model` - Optional `provider/model` used once when compaction exceeds its context window, reaches its output limit, or finishes successfully without summary text. The original session model remains selected. Authentication, quota, transport failures, cancellation, and other finish reasons do not activate this fallback. A fallback equal to the primary model is skipped. If the fallback cannot produce a complete summary that fits the original session model, compaction stops without replacing the previous checkpoint.
