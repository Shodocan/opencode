...
- `fallback_model` - Optional `provider/model` used once when compaction exceeds its context window, reaches its output limit, or finishes successfully without summary text. The original session model remains selected. Authentication, quota, transport failures, cancellation, and other finish reasons do not activate this fallback. When the fallback is the primary model, it may retry an incomplete summary only if its effective output allowance is larger; the model's variant is preserved. Same-model input overflow is not retried. If the fallback cannot produce a complete summary that fits the original session model, compaction stops without replacing the previous checkpoint.
- `fallback_max_output_tokens` - Output allowance for the configured fallback, including reasoning (default: `32000`). Must be a positive integer; the model and runtime limits still apply. Primary compaction retains its `4096`-token cap. Increasing this allowance also reserves more context for output before dispatch. Only a successful finish with nonempty summary text can become a checkpoint, even when no fallback is configured.
- `reserved` - Token buffer for compaction. Leaves enough window to avoid overflow during compaction.
- `threshold` - Auto-compaction threshold as a fraction (0-1) of the model's context window. Automatic compaction triggers when the previous turn's token usage reaches this fraction of the context (default: `0.8`, i.e. 80% of the model's supported context).
- `thresholds` - Per-model auto-compaction thresholds as a fraction (0-1) of each model's context window, keyed by `"providerID/modelID"`. Overrides `threshold` for the matched model, e.g. compact a 1M-context model at 400k tokens with `"thresholds": { "deepseek/deepseek-chat": 0.4 }`.
