...
- `thresholds` - Absolute auto-compaction thresholds in tokens per model, keyed by `"providerID/modelID"`. Overrides `threshold` for the matched model, e.g. compact a 1M-context model at 400k tokens with `"thresholds": { "openai/gpt-5": 400000 }`.
