...
- `fallback_model` - Optional `provider/model` used once when compaction exceeds its context window. The original session model remains selected. Authentication, quota, transport failures, and cancellation do not activate this fallback. If the fallback also cannot fit the history, compaction stops without replacing the previous checkpoint.
